...
## ✅ Estado actual: migración terminada y limpia

Todo vive en **un solo lugar** (Bluehosting), sin proyectos de prueba sueltos por ahí.

- **Sitio principal**: `https://antojitosdulces.cl` (dominio definitivo, comprado y configurado)
...
- **Dados de baja** (ya no existen, se eliminaron): Netlify, subdominio `antojitosdulces.codingbase.cl`, proyecto de Supabase — todos eran temporales del período de pruebas antes del dominio definitivo.
...
- Landing completa: hero, sobre nosotros (con "Nuestra Promesa"), catálogo por ocasión (6 tarjetas), **catálogo interactivo de 18 sabores** con modal y descripciones reales, precios en modal (tabla 18 tortas x 4 tramos de personas, con acceso también desde una tarjeta junto al mapa), features, testimonios, slider de Instagram automático, mapa embebido con la dirección real, footer con contacto
- Formulario de cotización → guarda en Postgres (tablas `clientes` + `cotizaciones`) Y envía correo por Formspree (ambos a la vez)
- Campos del formulario: nombre, apellido, teléfono, dirección/población (opcionales), tipo evento, fecha, hora de entrega/retiro, cantidad personas (15/20/30/40), tipo de torta (18 opciones), mensaje
...
- SSL activo (AutoSSL/Let's Encrypt, gratis, renovación automática)
